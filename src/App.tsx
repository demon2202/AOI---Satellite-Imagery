...
function AOIItem({ feature, index, onRemove, onZoom }: AOIItemProps) {
  return (
    <div className="aoi-item" style={{ animationDelay: `${index * 40}ms` }}>
      <div className="aoi-item-bar" style={{ background: feature.color }} />
      <div className="aoi-item-left">
        ...
        <div>
          ...
          <div className="aoi-meta">
            {feature.type}{feature.area ? ` · ${fmtArea(feature.area)}` : ''}
          </div>
        </div>
      </div>
...
function NotifItem({ n, index, onClick }: NotifItemProps) {
...
interface StatCardProps { label: string; value: number; color: string; emoji: string }
function StatCard({ label, value, color, emoji }: StatCardProps) {
  return (
    <div className="stat-card" style={{ '--c': color } as React.CSSProperties}>
      <div className="stat-icon-wrap" style={{ background: `${color}18` }}>{emoji}</div>
...
export default function App() {
  ...
  const [settingsState, setSettingsState] = useState({ notifs: true, autoSave: true, showCoords: true, defaultZoom: 5, lang: 'en' })
  ...
  useEffect(() => {
    ...
    map.on('mousemove', (e: L.LeafletMouseEvent) => {
      setCoords(fmtCoords(e.latlng.lat, e.latlng.lng))
    })
    ...
    map.on('draw:created', (e: L.LeafletEvent) => {
      ...
      setNotifications(p => [{
        id: uid(), title: 'AOI Created',
        message: `${feature.name} added to map.`,
  ...
  useEffect(() => {
    ...
    features.forEach(f => {
      ...
      if (f.type === 'polygon')
        layer = L.polygon(f.coordinates as [number, number][], { color: f.color, fillOpacity: 0.25 })
      else if (f.type === 'rectangle')
        layer = L.rectangle(f.coordinates as [number, number][], { color: f.color, fillOpacity: 0.25 })
  ...
  useEffect(() => {
    const map = mapRef.current; const drawn = drawnRef.current
    if (!map || !drawn) return
  ...
  const clearAll = useCallback(() => {
    if (confirm('Clear all features?')) { setFeatures([]); toast('Cleared', 'info') }
  }, [toast])
  ...
  const exportGeoJSON = useCallback(() => {
    ...
    a.href = url; a.download = 'aoi.geojson'; a.click()
            ...
            <aside className="sidebar">
              ...
              <div className="sidebar-section">
                ...
                <div className="tool-actions">
                  {activeTool && (
                    <button className="btn-cancel" onClick={() => setActiveTool(null)}>Cancel</button>
                  )}
              ...
              {/* AOI List — ReactBits AnimatedList style */}
          ...
          <div className="full-view">
            <div className="view-container">
              ...
              <div className="settings-block">
                ...
                {([
                  { key: 'notifs' as const,     label: 'Notifications', desc: 'Receive in-app notifications' },
                  { key: 'autoSave' as const,   label: 'Auto-save',     desc: 'Persist features to localStorage' },
                  { key: 'showCoords' as const, label: 'Show Coords',   desc: 'Display cursor coordinates on map' },
                ] as const).map(s => (
              ...
              <div className="settings-block">
                ...
                <div className="settings-row">
                  ...
                  <button className="btn-danger" style={{ flex: 'none', padding: '7px 16px' }}
                    onClick={() => { if (confirm('Delete everything?')) { localStorage.clear(); window.location.reload() } }}>
                    Clear
                  </button>
                </div>
              </div>
              ...
              <div className="panel-header">
                ...
                <div className="panel-actions">
                  {notifications.length > 0 && (
                    <>
                      <button className="clear-link" onClick={markAllRead}>mark read</button>
